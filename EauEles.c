        ...
        for (int j = 0; j < NB_COLONNE; ++j) {
            if (eceCity->tabCase[i][j].Etat >= CABANE && eceCity->tabCase[i][j].Etat <= GRATTE_CIEL) {
                ...
                if (parcoursGraphe->dejaEcrit == false) {
                    DrawText(TextFormat("%d/%d", parcoursGraphe->consoEau, eceCity->batiment[parcoursGraphe->batiment-1].nbHabitantMax), eceCity->tabCase[i][j].pos.x, eceCity->tabCase[i][j].pos.y, 20, BLACK);
                ...
                if (parcoursGraphe->dejaEcrit == false) {
                    DrawText(TextFormat("Stokage : %d/5000", parcoursGraphe->consoEau), eceCity->tabCase[i][j].pos.x +20, eceCity->tabCase[i][j].pos.y, 20, BLACK);
                    parcoursGraphe->dejaEcrit = true;
                }
            }
        }
